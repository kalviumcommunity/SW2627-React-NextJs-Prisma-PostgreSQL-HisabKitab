...
export default function LedgerView({ initialContacts }) {
  ...
  const handleAddParty = async (newParty) => {
    ...
    const optimisticParty = { ...newParty, id: `temp-${Date.now()}` };
    setParties([optimisticParty, ...parties]);
    
    const result = await createContact(newParty);
    if (!result.success) {
      setParties(parties); // Revert on failure
  ...
  const toCollectList = parties.filter(item => parseFloat(item.balance) > 0);
  const toGiveList = parties.filter(item => parseFloat(item.balance) < 0);

  const totalToCollect = toCollectList.reduce((acc, curr) => acc + parseFloat(curr.balance), 0);
  const totalToGive = toGiveList.reduce((acc, curr) => acc + Math.abs(parseFloat(curr.balance)), 0);
            ...
            >
              ...
              <div className={styles.amountGroup}>
                <span className={`${styles.amount} ${activeTab === "COLLECT" ? styles.amountCollect : styles.amountGive}`}>
                  ₹ {Math.abs(parseFloat(item.balance)).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                </span>
